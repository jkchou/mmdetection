...
class BoxInstDataPreprocessor(DetDataPreprocessor):
    ...
    def get_images_color_similarity(self, inputs: Tensor,
        ...
        """Compute the image color similarity in LAB color space.
            input: (b,c,h,w) -> output: (b,c,8,h,w)\\
            8代表了周围8个像素的8个方向。
        """
        assert inputs.dim() == 4
        assert inputs.size(0) == 1
        #unfold操作：将卷积区域的的像素展成一维。input：（ N, C, H, W），output：（N,C×∏(kernel_size),L）L 是(H,W)根据kernel_size尺寸滑动裁剪后，得到的区块的数量。
        #通过unfold操作，提取周围8个像素颜色进行比较
